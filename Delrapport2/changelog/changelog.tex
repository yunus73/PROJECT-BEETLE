...
\begin{document}
	...
	\begin{array}{rrl}
		...
		20-04-15 & YO/TD/EG/CL & \vtop{\hbox{\strut \emph{Added~edit\-link~(it~is~still~incomplete,~but~it’s~meant~to~be}}\hbox{\strut \emph{the~admin\text{-}access~to~add~new~entries~in~the~database)}}} \\
		...
		21-04-15 &TD & Updated~the~database,~so~it~actually~shows~a~real~insect
	\end{array}
	$$
\end{document}